...
% --- START OF THE FIX ---
% This command manually turns off the ACM reference block and copyright info.
% The 'anonymous' flag should do this by itself, but we are forcing it.
\settopmatter{printacmref=false}

% This command removes the page numbers, which is also standard for review.
\settopmatter{printfolios=false} 

% This is a more aggressive command to kill the copyright footer text
...
\copyrightyear{}
\acmYear{}
\acmDOI{} % Replace with actual DOI
\acmConference{}
...
The HLS market is dominated by commercial tools from major EDA vendors, though several academic and open-source solutions exist. Table \ref{tab:hls_tools_comparison_wrap} provides a non-exhaustive overview of prominent HLS tools.
...
% start snippet
% We use tabularx to make the table fit the text width.
% The 'X' columns are flexible and wrap text automatically.
% We use >{\raggedright\arraybackslash} to make the text in the X columns 
% left-aligned, which looks better than fully justified for narrow columns.
...
\begin{itemize}
    ...
    \item \textbf{General Approach with HLS: } With High-Level Synthesis (HLS), the designer works at a higher level of abstraction, describing the bit-reversal permutation algorithmically in a language like C or C++. The HLS tool is then responsible for interpreting this behavioral model to generate the underlying hardware. As highlighted by \cite{HLSForFFT}, the structure of this behavioral model is critical to the quality of the resulting hardware. The work presents a generic and configurable FFT model that allows the HLS tool to explore a much broader architectural space. By leveraging different parallelization strategies outlined in the model, the HLS tool can automatically synthesize various implementations for the bit-reversal logic and memory structure, enabling a trade-off between low-complexity architectures and high-throughput, parallelized ones based on user-defined constraints. This abstracts away the need to manually design state machines or address calculation units, placing the emphasis on creating a flexible high-level model instead.
\cite{HLSForFFT}
\end{itemize}
